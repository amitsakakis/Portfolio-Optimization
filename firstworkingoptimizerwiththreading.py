...
##THIS THREADING DID SORT OF WORK, THE ONLY THING WAS THAT FRONTIER GOT SCREWED UP
...
def portfolio_simulation(tickers, returns, results, i):
...
def plot_efficient_frontier(tickers, returns):
    num_portfolios = 10000
    results = np.zeros((3, num_portfolios))
    threads = []
    
    for i in range(num_portfolios):
        thread = threading.Thread(target=portfolio_simulation, args=(tickers, returns, results, i))
...
if sum(initial_allocations) != 1.0:
    ...
    if start_date and end_date:
        ...
        plot_efficient_frontier(tickers, returns)
